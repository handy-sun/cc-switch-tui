...
pub(super) fn normalize_hermes_model_fields(model: &mut Value) {
    ...
    let context_length = model
        ...
        .or_else(|| model.remove("context_window"))
        .or_else(|| model.remove("contextWindow"))
        .or_else(|| model.remove("contextLength"));
    model.remove("context_window");
    model.remove("contextWindow");
    model.remove("contextLength");
    if let Some(context_length) = context_length {
        model.insert("context_length".to_string(), context_length);
    }

    let max_tokens = model
        .remove("max_tokens")
        .or_else(|| model.remove("maxTokens"));
    model.remove("maxTokens");
    if let Some(max_tokens) = max_tokens {
        model.insert("max_tokens".to_string(), max_tokens);
    }
}
